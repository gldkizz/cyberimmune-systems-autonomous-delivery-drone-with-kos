...
- [IPC-сообщения](#ipc-сообщения)
  ...
  - [ipc_messages_server_connector](#ipc_messages_server_connector)
    ...
    - [receiveSubscription()](#int-receivesubscriptionchar-topicchar-subscriptionuint32_t-subscriptionsize)
...
Выполняет запись сообщения в лог с указанием модуля, отправившего сообщение, и уровня логирования. Логирование выводит сообщение в консоль, записывает его в файл лога на карте памяти и передает на сервер ОрВД.
...
Публикует сообщение с указанной темой по протоколу MQTT. Ответ от сервера ОРВД не ожидается.

#### `int receiveSubscription(char* topic, char* subscription, uint32_t subscriptionSize)`

Проверяет наличие сообщения по указанной теме, полученного от сервера по протоколу MQTT. Если сообщение было получено, оно удаляется и больше не будет возвращаться данной функцией.